...
public class TouchController : MonoBehaviour
{
	...
	private VirtualJoystickNew leftJoystick; // script component attached to the left joystick's background image
    private int leftSideFingerID = 0; // unique finger id for touches on the left-side half of the screen

    void Start()
    {

		if (leftJoystickBackgroundImage.GetComponent<VirtualJoystickNew>() == null)
        {
            Debug.LogError("There is no LeftJoystick script attached to the Left Joystick game object.");
        }
        else
        {
			leftJoystick = leftJoystickBackgroundImage.GetComponent<VirtualJoystickNew>(); // gets the left joystick script
