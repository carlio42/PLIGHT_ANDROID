...
namespace MyNetwork
{
    public class FreeLookCam : PivotBasedCameraRig
    {
		...
		private int leftSideFingerID, RightFingerID;
			...
			if (Input.touchCount > 0)
			{
				...
				for (int i = 0; i < Input.touchCount; i++)
				{
					...
					if (myTouches[i].phase == TouchPhase.Began)
					{
						...
						{
							RightFingerID = myTouches [i].fingerId;
						}

					}
					if (myTouches [i].phase == TouchPhase.Moved) 
					{
						if (myTouches [i].position.x > Screen.width / 2 && RightFingerID == myTouches [i].fingerId)
						{
							HandleRotationMovement ();
						}
						else if (myTouches [i].position.x < Screen.width / 2 && leftSideFingerID == myTouches [i].fingerId) 
						{
							// Do Nothing
						}
					}
					...
					if (myTouches[i].phase == TouchPhase.Ended)
					{
						...
						if (myTouches[i].fingerId == RightFingerID)
						{
							RightFingerID = -1;
						}
					}
		...
		//if i find solution for direct touch input, i will change it to :-
		//public void HandleRotationMovement(float x, float y)
		//and in Update function remove the HandleRotationMovement();
